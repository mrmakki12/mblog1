...
app.delete('/api/v1/articles/:id/delete', authenticated, async (req, res, next) => {
    ...
    db.query(`DELETE FROM comments WHERE article_id = ?`, req.params.id, (err, result) => {
        if(err) next(err);
    });
 ...
 **/ 
app.get('/:route', authenticated, (req, res) => {
    res.sendFile(__dirname + `frontend/build/index.html/${req.params.route}`);
});
